...
samples are taken using a combination of three new C-APIs in ruby 2.1:
  ...
  - in allocation mode, stack frames are collected via `rb_tracepoint_new(RUBY_INTERNAL_EVENT_NEWOBJ)`,
    which is triggered every time the VM allocates a new object.
...
for each frame in a sample, the profiler collects some metadata:
  ...
  - total_samples: samples where this frame is in the stack
  - lines: samples per line for this frame
  - edges: samples per callee (methods invoked by this frame)
...
the profiler is compiled as a C-extension and exposes one api: `StackProf.run`. `run` takes a block of code
and returns a profile in the format below.
...
this data structure is part of the public API and is intended to be saved (as json/marshal perhaps) for later
processing. the reports above can be generated by passing this structure into `StackProf::Report.new`.
