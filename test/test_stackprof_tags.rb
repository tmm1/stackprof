...
class StackProfTagsTest < MiniTest::Test
  ...
  def tag_order_matches(profile, order)
    ...
    profile.fetch(:sample_tags, []).each do |tags|
      ...
      if tags != acceptable && idx < order.size
        ...
        debugstr += sprintf("%02d/%02d) %s != %s, next %s\n", idx, order.size, tags, acceptable, next_acceptable)
        if tags != next_acceptable
          break
        end
  ...
  def test_sample_tag_persistence_from_parent
    ...
    profile = StackProf.run(mode: :cpu, tags: [:thread_id, :foo, :spam]) do
      Thread.new do
        StackProf::Tag.set(foo: :baz)
    ...
    #STDERR.puts "PROF #{profile[:sample_tags].inspect}"
    assert_equal true, tag_order_matches(profile, [{thread_id: sub_id, foo: :baz, spam: :eggs}, {thread_id: main_id, foo: :bar, spam: :eggs}, {thread_id: main_id}])
    ...
    #STDERR.puts "PROF #{profile[:sample_tags].inspect}"
    assert_equal true, tag_order_matches(profile, [{thread_id: sub_id, foo: :baz}, {thread_id: main_id, foo: :bar, spam: :eggs}, {thread_id: main_id}])
  end
