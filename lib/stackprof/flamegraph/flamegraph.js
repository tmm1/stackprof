...
function OverviewFlamechart(container, viewportOverlay, data, dataRange, info) {
  ...
  self.viewportOverlay.addEventListener('mouseup', self.onOverlayMouseUp.bind(self))
  self.viewportOverlay.addEventListener('mousemove', self.onOverlayMouseMove.bind(self))
}
...
OverviewFlamechart.prototype.onMouseDown = function(e) {
  ...
  self.viewportOverlay.classList.add('clickthrough')
...
OverviewFlamechart.prototype.onMouseUp = function(e) {
  ...
  self.dragging = false
  self.viewportOverlay.classList.remove('clickthrough')
...
OverviewFlamechart.prototype.handleDragGesture = function(e) {
  ...
  if (self.dragCurrentX === e.offsetX)
    return

  self.dragCurrentX = e.offsetX
  ...
  rect.y = Math.max(self.viewport.y, Math.min(self.viewport.height - self.viewport.y, e.offsetY / self.height * self.viewport.height + self.viewport.y - rect.height / 2))
