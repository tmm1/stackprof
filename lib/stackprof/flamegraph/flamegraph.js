...
Flamechart.prototype.paint = function(opacity, frames, gemName) {
  ...
  frames.forEach(function(f) {
    ...
    ctx.fillStyle = colorString(i.color, opacity)
    ...
    if (!self.showLabels || r.width < 15)
      return

    ctx.fillStyle = '#000'

    var metrics = ctx.measureText(f.frame)
    if (metrics.width * 0.75 < r.width) {
      // Let the text squeeze to fit.
      ctx.fillText(f.frame, r.x + 1, r.y + r.height / 2, r.width - 2)
    } else {
      var text = truncateToWidth(f.frame, r.width, function(t) { return ctx.measureText(t).width })
      ctx.fillText(text, r.x + 1, r.y + r.height / 2, r.width - 2)
    }
  })
}
...
function MainFlamechart(canvas, data, dataRange, info) {
  ...
  self.canvas.addEventListener('mouseup', self.onMouseUp.bind(self))
...
MainFlamechart.prototype.onMouseMove = function(e) {
  ...
  if (self.dragging) {
    var viewport = Object.assign({}, self.viewport)
    viewport.x = self.dragInfo.viewport.x - (e.offsetX - self.dragInfo.mouse.x) * viewport.width / self.width
    viewport.y = self.dragInfo.viewport.y - (e.offsetY - self.dragInfo.mouse.y) * viewport.height / self.height
  ...
  var frame = self.frameAtPoint(e.offsetX, e.offsetY)
...
MainFlamechart.prototype.onWheel = function(e) {
  ...
  if (e.shiftKey) {
    ...
    self.handleZoomGesture(Math.pow(1.2, (e.deltaY || -e.deltaX) * mouseWheelZoomSpeed), e.offsetX)
    e.preventDefault()
    return
  }

  var viewport = Object.assign({}, self.viewport)
  viewport.x += e.deltaX * viewport.width / (self.dataRange.maxX - self.dataRange.minX)
  viewport.x = Math.min(self.dataRange.maxX - viewport.width, Math.max(self.dataRange.minX, viewport.x))
  viewport.y += (e.deltaY / 8) * viewport.height / (self.dataRange.maxY - self.dataRange.minY)
  viewport.y = Math.min(self.dataRange.maxY - viewport.height, Math.max(self.dataRange.minY, viewport.y))
  self.setViewport(viewport)
  e.preventDefault()
}
...
function OverviewFlamechart(canvas, viewportOverlay, data, dataRange, info) {
  var self = this
...
function FlamegraphView(data, info, sortedGems) {
  ...
  self.overview = new OverviewFlamechart(document.querySelector('.overview'), document.querySelector('.overview-viewport-overlay'), data, self.dataRange, info)
...
function truncateToWidth(text, maxWidth, textWidth) {
  var originalWidth = textWidth(text)
  if (originalWidth <= maxWidth)
    return text

  var shortestWidth = textWidth(centerTruncate(text, 0))
  ...
  while (minLength < maxLength) {
    var length = Math.ceil((maxLength + minLength) / 2)
    var truncated = centerTruncate(text, length)
    var width = textWidth(truncated)
    if (width <= maxWidth) {
      minLength = length
    } else {
      maxLength = length - 1
  ...
  return centerTruncate(text, minLength)
}
